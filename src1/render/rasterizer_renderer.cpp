...
FrameBuffer Context::frame_buffer(0, 0);

FrameBuffer::FrameBuffer(int w, int h) : width(0), height(0) { resize(w, h); }
...
void RasterizerRenderer::render(const Scene& scene)
{
    ...
    Context::frame_buffer.resize(Uniforms::width, Uniforms::height);
    ...
    for (const auto& group: scene.groups) {
        for (const auto& object: group->objects) {
            ...
            workers.reserve(
                static_cast<std::size_t>(vertex_threads + rasterizer_threads + fragment_threads)
            );
    ...
    std::chrono::duration<double> rendering_duration = end_time - begin_time;
...
{
    std::vector<VertexShaderPayload> batch;
    batch.reserve(128);
    ...
    while (true) {
...

        {
            ...
            while (!vertex_queue.empty() && batch.size() < 128) {
...
void FragmentProcessor::worker_thread()
{
    ...
    while (true) {
...

        {
            ...
            while (!Context::rasterizer_output_queue.empty()) {
                batch.push_back(Context::rasterizer_output_queue.front());
                Context::rasterizer_output_queue.pop();
            }
        ...
        for (auto& fragment: batch) {
            ...
            Eigen::Vector3f shaded_color;
            bool            updated = Context::frame_buffer.set_pixel_lazy(
                index, fragment.depth,
                [&]() {
                    shaded_color = fragment_shader_ptr(
