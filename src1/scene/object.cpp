...
Matrix4f Object::model()
{
    ...
    Rx(1, 1) = std::cos(x_rad);
    Rx(1, 2) = -std::sin(x_rad);
    Rx(2, 1) = std::sin(x_rad);
    Rx(2, 2) = std::cos(x_rad);
    
    Matrix4f Ry = Matrix4f::Identity();
    Ry(0, 0) = std::cos(y_rad);
    Ry(0, 2) = std::sin(y_rad);
    Ry(2, 0) = -std::sin(y_rad);
    Ry(2, 2) = std::cos(y_rad);
    
    Matrix4f Rz = Matrix4f::Identity();
    Rz(0, 0) = std::cos(z_rad);
    Rz(0, 1) = -std::sin(z_rad);
    Rz(1, 0) = std::sin(z_rad);
    Rz(1, 1) = std::cos(z_rad);
    
    // ZYX欧拉角旋转矩阵：R = Rx(θx) * Ry(θy) * Rz(θz)
    Matrix4f rotation_matrix = Rx * Ry * Rz;
